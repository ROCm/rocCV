...
- Moved the benchmark suite's nlohmann/json copy from `benchmarks/vendor/` to `vendor/` and pinned it to the v3.12.0 release; the previous copy was an unpinned post-3.12.0 `develop` snapshot that reported itself as 3.12.0, and it shipped without its license file. See `vendor/ORIGIN.md` for provenance.
